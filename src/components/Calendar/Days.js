 ...
 const getEvents = (d) => {
  ...
  if (props.monthsData[props.month][d.toString()]) {
   ...
   if (daysData.length > 3) {
    ...
    events[2] = <IconItem key="2" className="more-event-element" icon="arrowsmallright" labelPosition="before">Click to view more</IconItem>;
   } else {
 ...
 for (let d = 1; d <= props.daysInMonth(); d++) {
  ...
  if (props.monthsData && props.monthsData[props.month]) {
   daysInMonth.push(
    ...
    >
     <Item centered className="day-text">{d}</Item>
     {getEvents(d)}
    </td>
   );
  } else {
